...
import { convertDimensionValue, resolveTokenReference } from "./converters";
...
function extractLayoutValue(token: SpectrumTokenValue, scale: "desktop" | "mobile" = "desktop"): string | null {
...
function buildReferenceMap(): Map<string, string> {
    ...
    const tokens = loadTokenFile("layout.json");
    ...
    for (const [name, tokenValue] of Object.entries(tokens)) {
        ...
        const desktopValue = extractLayoutValue(token, "desktop");
        if (desktopValue) {
...
function generateLayoutCSS(allTokensMap: Map<string, string>): string {
    const tokens = loadTokenFile("layout.json");
    const lines: string[] = [];
    ...
    for (const [name, tokenValue] of Object.entries(tokens)) {
        ...
        // Skip non-layout tokens
        if (!isLayoutToken(token.$schema)) {
            continue;
        }

        // Extract values for all scales
        for (const scale of ["desktop", "mobile"] as const) {
            const value = extractLayoutValue(token, scale);
            if (value) {
                // Resolve token references like {corner-radius-100}
                const resolvedValue = resolveTokenReference(value, allTokensMap);
                const cssValue = convertDimensionValue(resolvedValue);
...
function main() {
    console.log("Loading Spectrum layout tokens...");
    ...
    // Generate CSS
    const css = generateLayoutCSS(allTokensMap);
    const outputPath = join(outputDir, "layout.css");
    ...
    console.log(`\n✓ Layout CSS generated in: ${outputDir}`);
}
